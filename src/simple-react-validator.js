class SimpleReactValidator {
  ...
  constructor(options = {}) {
    ...
    this.rules = {
      ...
      between              : {message: 'The :attribute must be between :min and :max:type.',                    rule: (val, params) => this.helpers.size(val) >= parseFloat(params[0]) && this.helpers.size(val, params[2]) <= parseFloat(params[1]), messageReplace: (message, params) => message.replace(':min', params[0]).replace(':max', params[1]).replace(':type', this.helpers.sizeText(params[2]))},
      ...
      max                  : {message: 'The :attribute may not be greater than :max:type.',                     rule: (val, params) => this.helpers.size(val) <= parseFloat(params[0]), messageReplace: (message, params) => message.replace(':max', params[0]).replace(':type', this.helpers.sizeText(params[1]))},
      min                  : {message: 'The :attribute must be at least :min:type.',                            rule: (val, params) => this.helpers.size(val) >= parseFloat(params[0]), messageReplace: (message, params) => message.replace(':min', params[0]).replace(':type', this.helpers.sizeText(params[1]))},
      ...
      size                 : {message: 'The :attribute must be :size:type.',                                    rule: (val, params) => this.helpers.size(val) === parseFloat(params[0]), messageReplace: (message, params) => message.replace(':size', params[0]).replace(':type', this.helpers.sizeText(params[1]))},
  ...
  helpers = {
    ...
    size(val) {
      ...
      if (['array', 'string', 'object'].includes(typeof val)) {
        return val.length;
      }
